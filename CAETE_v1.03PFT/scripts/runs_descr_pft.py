...
def read_as_array(nc_fname, var):
    ...
    #fcon = nc.Dataset(nc_fname)
        data_array = fcon.variables[var][:].data
    return np.fliplr(data_array)


def make_table_aux(folder):
    
    traits = ['tleaf','tawood','tfroot','aleaf','aawood','afroot']
    ...
    attr_table = pd.read_csv('pfts.csv',dtype=np.float32)  
    os.chdir(folder)
...
def make_table():
    ...
    flds = ["/home/bianca/paper_mestrado_codes_results/CAETE_versions/CAETEv1.0_3PFT/scripts/CAETEv1.0_3PFT_lowprec"]
